...
if($_GET["id"] != ""){
    ...
    // $direccion     = mb_convert_encoding($fila["pac_direccion"],'UTF-8','ISO-8859-1');
    // $telefono      = mb_convert_encoding($fila["pac_telefono"],'UTF-8','ISO-8859-1');
    // $fecha_nac     = empty($fila["pac_fechanac"])? "" : fechaEU($fila["pac_fechanac"]);
    // $edad          = empty($fila["pac_fechanac"])? $fila["pac_edad"] : calculaedad($fila["pac_fechanac"]);
    // $observaciones = mb_convert_encoding($fila["pac_observaciones"],'UTF-8','ISO-8859-1') . "\r" . date("d-m-y");
